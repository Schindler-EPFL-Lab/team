...
def main():
    ...
    parser.add_argument("--index", type=int, default=1, help='Index of reproduction trajectory')
    ...
    if task not in ["rail_cleaning", "brush_homing", "brush_picking_up", "opening"]:
        raise ValueError("task name should be in [rail_cleaning, brush_homing, brush_picking_up, opening]")
    ...
    for i in range(np.shape(regression)[1] - 1):
        axs[x, y].plot(regression[:, 0], regression[:, i + 1], linewidth=5, label="regression")
        axs[x, y].plot(regression[:, 0], reproduction[:, i], linewidth=5, label="reproduction")
