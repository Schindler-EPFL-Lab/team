...
class Trajectory:
    ...
    def joints(self) -> np.ndarray:
        return self.trajectory[:, 1:7]

    @property
    def tcp(self) -> np.ndarray:
        return self.trajectory[:, 7:10]
    ...
    def joint_timestamps(self) -> np.ndarray:
        return self.trajectory[:, :7]

    @property
    def average_sampling(self) -> float:
        return len(np.unique(self.trajectory, axis=0)) / self.timestamps[-1]
    ...
    def upsample(self, des_freq: int) -> None:
        ...
        self.trajectory[:, 0] = self.timestamps - self.timestamps[0]
        ...
        self.trajectory = np.append(time_new.reshape(-1, 1), joints_new, axis=1)
